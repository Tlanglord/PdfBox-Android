...
public final class PDResources implements COSObjectable {
    ...
    public PDFont getFont(COSName name) throws IOException {
        ...
        Log.d(TAG, "getFont: 从资源Resources中获取字体资源 ， 字体=" + name.getName());

        COSObject indirect = getIndirect(COSName.FONT, name);

        Log.d(TAG, "getFont: indirect , 从资源Resources中获取字体资源 ， 字体=" + indirect);
...
//        Log.d(TAG, "getFont: base, 从资源Resources中获取字体资源 ， 字体=" + base);
        if (base instanceof COSDictionary) {

            String fontName = "";
            try {
                COSBase baseFont = ((COSDictionary) base).getDictionaryObject("BaseFont");
                if (baseFont instanceof COSName) {
                    fontName = ((COSName) baseFont).getName();
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
            Log.d(TAG, "getFont: base, 从资源Resources中获取字体资源 ， 字体=" + fontName);
            font = PDFontFactory.createFont((COSDictionary) base, cache);

            Log.d(TAG, "getFont: 获取到字体是:" + font.getName());
        }

...
/**
 * ASCII85，也被称为Base85，是一种二进制到文本的编码方法，用于将二进制数据转换为ASCII字符。这种编码方法主要用于电子邮件和网络传输，以便在不支持二进制数据的系统中传输数据。
 * <p>
 * ASCII85编码方法的基本原理是将每4个字节（32位）的二进制数据转换为5个ASCII字符。这5个字符的ASCII值在33（'!'）到117（'u'）之间，因此得名“ASCII85”。
 * <p>
 * ASCII85编码比Base64编码更加紧凑，可以节省存储空间和传输带宽。例如，对于同样的二进制数据，ASCII85编码后的文本比Base64编码后的文本小约25%。
 * <p>
 * 在PDF文件中，ASCII85编码常用于对图像数据和二进制流进行编码。例如，PDF的图像XObject和嵌入的字体文件通常会使用ASCII85编码。
